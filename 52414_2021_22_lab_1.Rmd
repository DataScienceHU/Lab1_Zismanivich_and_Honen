...
cross_corr <- function(corr_df, country, col1, col2){
  ...
  col1 <- as.numeric(as.list(t(corr_df%>%filter(location = country_name[, col1]))))
  col2 <- as.numeric(as.list(t(corr_df%>%filter(location = country_name[, col2]))))
  for (i in c(-60:60)) {
    if (i< 0){
      new_vec <- append(new_vec, cor(col1[(abs(i) + 1):length(col1)], col2[1:length(col1)+i]))
      
    }
    
    else if (i>=0){
      new_vec <- append(new_vec, cor(col2[i+1:length(col1)]), col1[1:length(col1)-i])
    }
...
box_continent_2 =  covid_19 [, c("location" ,"continent","death_rate", "date")] 
box_continent_2$continent =  ifelse(covid_19$location == "World","World",covid_19$continent)
box_continent_2 = box_continent_2 %>% filter(continent != "")
