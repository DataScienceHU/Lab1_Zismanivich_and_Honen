...
covid_19 = read.csv("C:/R/Datasets/owid-covid-data.csv")
...
```{r}
month_ds =  subset(covid_19, date> "2019-12-31" & date < "2022-04-01" ) #setting dates
month_ds = month_ds[,c("location", "date", "new_cases_per_million" )]#subseting
month_ds = month_ds %>% group_by(date= format(as.Date(date),"%y-%m")) %>% group_by(date,location) %>% summarise(new_cases_per_million_month = sum(new_cases_per_million)) # orgenized by month
#using boxplot func.
...
```{r}
month_ds_death =  subset(covid_19, date> "2019-12-31" & date < "2022-04-01" ) #data by date
month_ds_death = month_ds_death[,c("location", "date", "new_deaths" )] #subseting
month_ds_death = month_ds_death %>% group_by(date= format(as.Date(date),"%y-%m")) %>% group_by(date,location) %>% summarise(new_deaths_month = sum(new_deaths)) #data by month
#usin box plot func
...
```
```{r}
month_ds_vacc =  subset(covid_19, date> "2019-12-31" & date < "2022-04-01" ) # setting period of time.
month_ds_vacc = month_ds_vacc[,c("location", "date", "new_vaccinations" )] #subset
month_ds_vacc = month_ds_vacc %>% group_by(date= format(as.Date(date),"%y-%m")) %>% group_by(date,location) %>% summarise(new_vacc_month = sum(new_vaccinations)) #organized the data by month.
#creating box plot func.
...
```{r}
#creating new data and calculate 
...
covid_19$R_cases = r_case_df$R_cases #saving the column in the main data.
```
...
country_data =  covid_19 [, c("location", "date", "R_cases")] %>%  subset( location == "Israel" |location == "United States"| location == "United Kingdom")# Making a subset for the relevant data.
ggplot(country_data, aes(x = date, y= R_cases)) + 
  geom_line(aes( color= (location))) 

country_data <- fill(country_data,everything()) #gerrid NAS

country_data_2 = country_data %>% group_by(location) %>% filter(R_cases >=1)  #filter r_cases date under 1

#creating values for matrix for 3 countries
...
current %>% top_n(3, total_vaccinations_per_hundred) %>% select(location, total_vaccinations_per_hundred )#using topn func
...
box_continent_2 =  covid_19 [, c("location" ,"continent","death_rate", "date", "total_deaths","total_cases")] 
box_continent_2$continent =  ifelse(covid_19$location == "World","World",covid_19$continent)
box_continent_2 <- box_continent_2 %>% filter(continent != "") %>% group_by(continent,date) %>% summarise(world_total_rate_d = sum(total_deaths,na.rm = TRUE)/sum(total_cases,na.rm = TRUE))
box_continent_2 <- box_continent_2 %>% filter(date >= "2021-01-01")

ggplot(box_continent_2, aes(x = date, y= world_total_rate_d)) + 
  geom_smooth(aes( color= (continent))) 
...
ggplot(box_continent_2, aes(x = date, y= total_vaccinations_per_hundred)) + 
  geom_smooth(aes( color= (continent))) 
